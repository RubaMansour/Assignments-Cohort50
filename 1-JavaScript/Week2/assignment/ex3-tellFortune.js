...
export function tellFortune(numKids,partnerNames,locations,jobTitles) {
  ...
  return `You will be a ${selectJobTitles} in ${selectLocations}, married to ${selectPartnerNames} with ${selectNumKids} kids.`;
}

function main() {
  const numKids = [1, 2, 3, 4, 5];
  const partnerNames = ['Ahmed','Mohammed','Ali','Adam','Omar'];
  const locations = ['Amsterdam','Dubai','Egypt','Oman','Al Doha'];
